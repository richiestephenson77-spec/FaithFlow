      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          {user.isVerifiedPastor && (
            <ShieldCheck size={13} color="#C0603F" strokeWidth={2} className="flex-shrink-0" />
          )}
        </div>
        {user.churchName && (
          <p className="text-xs font-medium truncate mt-0.5" style={{ color: '#C0603F' }}>{user.churchName}</p>
        )}
        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
          ...
          {user.prayerWarriorBadge && (
            <span className="flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full"
              style={{ background: 'rgba(201,147,47,0.1)', color: '#C0603F' }}>
      ...
      <motion.button
        ...
        className={`flex-shrink-0 text-xs font-medium px-3 py-1.5 rounded-full transition-colors ${
          user.isFollowedByMe
            ...
            : 'border border-gray-200 text-gray-700 hover:border-[#C0603F]/40 hover:text-[#C0603F]'
        }`}
      >
